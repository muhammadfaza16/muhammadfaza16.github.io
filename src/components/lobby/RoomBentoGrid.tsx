...
interface DockIconProps {
    ...
    delay?: number;
    guarded?: boolean;
    onGuardedClick?: (href: string) => void;
}

const DockIcon = ({ title, href, icon, iconColor, delay = 0, guarded, onGuardedClick }: DockIconProps) => {
    const iconContent = (
        <motion.div
    ...
    if (guarded && onGuardedClick) {
        return (
            <div onClick={() => onGuardedClick(href)} style={{ textDecoration: 'none', cursor: 'pointer' }}>
                {iconContent}
            </div>
        );
    }
...
export function RoomBentoGrid({ onGuardedClick }: RoomBentoGridProps) {
        ...
        <div style={{
            ...
            <nav style={{
                ...
                <div style={{
                    ...
                    {apps.map((app, idx) => (
                        <DockIcon key={idx} {...app} delay={0.3 + idx * 0.06} onGuardedClick={onGuardedClick} />
                    ))}
                </div>
            </nav>
        </div>

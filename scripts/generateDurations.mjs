...
async function main() {
    console.log("Starting duration generation for " + PLAYLIST.length + " tracks...");
    const outData = [];

    for (const song of PLAYLIST) {
        try {
            const rawFileName = decodeURIComponent(song.audioUrl).replace('/audio/', '');
            const filePath = path.join(process.cwd(), 'public', 'audio', rawFileName);
            ...
            console.log(`Processed: ${song.title} -> ${duration}s`);
        } catch (e) {
            console.error("Failed for " + song.title, e.message);
        }
    }

    const outPath = path.join(process.cwd(), 'src', 'data', 'radioPlaylist.json');
    fs.mkdirSync(path.dirname(outPath), { recursive: true });
    fs.writeFileSync(outPath, JSON.stringify(outData, null, 2), 'utf-8');

    console.log("Successfully wrote durations to " + outPath);
}
